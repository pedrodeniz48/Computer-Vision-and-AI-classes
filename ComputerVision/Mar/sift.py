...
def generateGaussianImages(img, octaves, gaussian_kernels):
    ...
    for i in range(octaves):
        ...
        for gaussian_kernel in gaussian_kernels[1:]:
            imgGB = cv2.GaussianBlur(image, (0, 0), sigmaX=gaussian_kernel, sigmaY=gaussian_kernel)
...
def computeKeypoints(keypoint, octave_index, gaussian_image, radius_factor=3, bins=36, peak_ratio=0.8, scale_factor=1.5):
    ...
    for n in range(bins):
        smooth_histogram[n] = (6 * raw_histogram[n] + 4 * (raw_histogram[n - 1] + raw_histogram[(n + 1) % bins]) + raw_histogram[n - 2] + raw_histogram[(n + 2) % num_bins]) / 16.
...
def generateDescriptors(keypoints, gaussian_images, window_width=4, bins=8, scale_multiplier=3, descriptor_max_value=0.2):
    ...
    for keypoint in keypoints:
        ...
        thrs = norm(descriptor_vector) * descriptor_max_value
        descriptor_vector[descriptor_vector > thrs] = thrs
        descriptor_vector /= max(norm(descriptor_vector), 0.0001)
...
img = cv2.imread('eiad.jpeg', 0)   
kpoints, descriptors = computeSIFT(img)